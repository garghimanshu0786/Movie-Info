...
class RemoveMovieFromPlaylistUseCase @Inject constructor(private val movieRepository: IMovieRepository) {
		...
		if (id != null) {
			val updatedMovies = movieEntity.movies.map { movie ->
				if (movie.id == id && playlistName in movie.playlists) {
					movieRepository.removeMovieFromPlaylist(id, playlistName)
